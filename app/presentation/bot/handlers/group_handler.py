...
from app.presentation.ui.keyboards import get_cuisine_keyboard
...
async def start_command(message: types.Message, state: FSMContext):
    ...
    await message.answer(ASK_CUISINE, reply_markup=get_cuisine_keyboard())
...
async def set_food_preferences(message: types.Message, state: FSMContext):
    """ Устанавливаем предпочтения по конкретным продуктам (любимые, аллергии и нелюбимые продукты) """
