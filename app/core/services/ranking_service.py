...
class RankingService:
    async def __get_rrf_score_for_user(
        ...
        rrf_coef: int = 3, significance_coef: np.ndarray[float] = np.array([0.282, 0.308, 0.231, 0.179])
    ) -> np.ndarray[float]:
        ...
        matrix = np.array([
            ...
            # (await map_api.get_route(Point(lat=user.base_position_lat, lon=user.base_position_lng), current_place.point)).distance,
    ...
    async def __get_places_score(self, init_user: User, users: list[User], places: list[PlaceInfo]) -> np.ndarray[float]:
        ...
        matrix = np.array([
            await self.__get_rrf_score_for_user(user, places, distances) for user in users
        ])
